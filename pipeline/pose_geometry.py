...
# Leffa/OpenPose ordering: nose, neck, R shoulder/elbow/wrist,
# L shoulder/elbow/wrist, R hip/knee/ankle, L hip/knee/ankle, eyes, ears.
NOSE = 0
NECK = 1
...
def openpose_points(
    ...
    source_size: tuple[int, int] = (384, 512),
) -> tuple[np.ndarray, np.ndarray]:
    """Convert Leffa OpenPose output to Nx2 points plus a visibility mask."""
...
def _length(points: np.ndarray, a: int, b: int) -> float:
    return float(np.linalg.norm(points[b] - points[a]))


def _safe_length(points: np.ndarray, valid: np.ndarray, a: int, b: int) -> float | None:
...
def assess_pose_pair(
    base_points: np.ndarray,
    base_valid: np.ndarray,
    donor_points: np.ndarray,
    donor_valid: np.ndarray,
) -> PoseDiagnostics:
    """Decide whether skeleton-based DensePose retargeting is conservative enough."""
...
def _desired_length(
    ...
    b: int,
    strength: float,
    ratio_limits: tuple[float, float] = (0.72, 1.38),
) -> float | None:
...
def retarget_skeleton(
    ...
    """Preserve donor joint directions while moving bone lengths toward the base person."""
    strength = float(np.clip(strength, 0.0, 1.0))
    out = np.asarray(donor_points, dtype=np.float32).copy()
    if strength <= 0.0:
        return out

    # Keep the donor neck/head orientation. Retarget shoulders outward from neck.
    ...
    # Arms follow the newly positioned shoulders while retaining donor directions.
    for parent, child in (
        (R_SHOULDER, R_ELBOW),
        (R_ELBOW, R_WRIST),
        (L_SHOULDER, L_ELBOW),
    ...
    # Retarget pelvis center along the donor torso direction, then hip spacing.
    ...
    # Legs follow the retargeted hips.
    for parent, child in (
        (R_HIP, R_KNEE),
        (R_KNEE, R_ANKLE),
        (L_HIP, L_KNEE),
...
def _dedupe_control_points(
    src: Iterable[np.ndarray],
    dst: Iterable[np.ndarray],
...
def _triangle_indices(tri: np.ndarray, points: np.ndarray, tolerance: float = 2.5) -> tuple[int, int, int] | None:
    ids: list[int] = []
...
def piecewise_affine_warp(
    ...
    """Warp a control image with Delaunay triangles and fixed border anchors."""
    ...
    border = np.asarray(
        [
            ...
            [w * 0.5, 0], [w - 1, h * 0.5], [w * 0.5, h - 1], [0, h * 0.5],
        ],
        dtype=np.float32,
    )
    ...
    for x, y in src_all:
        # Subdiv2D rejects points exactly on max bounds in some OpenCV builds.
    ...
    # Preserve any rare uncovered pixels; border anchors should make this small.
    output[coverage == 0] = arr[coverage == 0]
    return Image.fromarray(np.clip(output, 0, 255).astype(np.uint8))


def retarget_densepose_control(
    ...
    donor_valid: np.ndarray,
    strength: float = 0.65,
) -> tuple[Image.Image, np.ndarray, PoseDiagnostics]:
    """Retarget real DensePose IUV using a conservative OpenPose skeleton warp."""
    ...
    common = base_valid & donor_valid
    # Facial landmarks are kept unchanged by retarget_skeleton and act as head anchors.
    ...
    if len(displacement) and float(np.percentile(displacement, 90)) > 140.0:
        diagnostics = PoseDiagnostics(
            diagnostics.common_body_joints,
            diagnostics.completeness,
            diagnostics.proportion_gap,
            False,
            "retarget displacement is too large",
        )
        return densepose.convert("RGB"), target, diagnostics
...
def draw_skeleton(
    ...
    """Debug overlay for measured/retargeted OpenPose landmarks."""
